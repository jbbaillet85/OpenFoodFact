...
class Substitute(ConnexionBDD):
    def __init__(self, substituted_id, user, password, host, database):
        ...
        query_get_store_name = f"""SELECT store_name FROM store
        WHERE store_id = '{self.substitut_store_id}'"""
        self.substitut_store_name = self.get_attribut(query_get_store_name)
        query_get_subtituted_nutriscore = f"""SELECT food_nutriscore FROM food
        WHERE food_id = '{self.substituted_id}'"""
        self.substituted_nutriscore = self.get_attribut(query_get_subtituted_nutriscore)
        query_get_substitut_nutriscore = f"""SELECT food_nutriscore FROM food
    ...
    def print_substitute(self):
        """function displaying the substitute with its nutriscore, its web page,
        and the store where to buy it
        """
        print(f"{self.category_id}-{self.category_name}")
        print(f"""A la place du produit {self.substituted_name} d'un nutriscore
        de {self.substituted_nutriscore}, vous pouvez consommer le produit de substitution
        {self.substitute_id}-{self.substitut_name} avec un nutriscore de {self.substitut_nutriscore}""")
    ...
    def save_substitute(self):
        ...
        """
        save = input(f"""Souhaitez vous enregistrer votre produit
        de substitution: {self.substitut_name} dans vos favoris? o/n : """)
        if save == "o":
            query_select = f"""SELECT id_substited FROM substitute
            WHERE id_substited = '{self.substituted_id}'"""
            self.cursor.execute(query_select)
            if self.cursor.rowcount == -1:
                self.cursor.fetchall()
            if self.cursor.rowcount == 0:
                query_save = f"""INSERT INTO Substitute (id_substitute, id_substited)
                VALUES ('{self.substitute_id}' , '{self.substituted_id}') """
